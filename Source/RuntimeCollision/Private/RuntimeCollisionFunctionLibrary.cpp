...
bool URuntimeCollisionFunctionLibrary::GenerateConvexCollisionForStaticMesh(UStaticMesh* StaticMesh, int32 HullCount, int32 MaxHullVerts, int32 HullPrecision)
{
	...
	// Run actual until to do the work (if we have some valid input)
	DecomposeMeshToHulls(StaticMesh->GetBodySetup(), Verts, CollidingIndices, HullCount, MaxHullVerts, HullPrecision);
